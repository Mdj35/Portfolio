    ...
    <div ref={containerRef} style={{ height: '400vh', position: 'relative' }}>
      ...
      <div style={{ position: 'sticky', top: 0, height: '100vh', width: '100%', overflow: 'hidden', background: 'var(--bg-primary)' }}>
        ...
        <motion.div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: floatOpacity, zIndex: 1, pointerEvents: 'none' }}>
          {/* Top Left, slow */}
          <motion.img src={sample} style={{ position: 'absolute', top: '10%', left: '5%', width: '300px', borderRadius: '16px', y: img1Y, rotate: img1Rotate, filter: 'blur(2px) brightness(0.7)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }} />
          
          {/* Bottom Right, medium */}
          <motion.img src={church} style={{ position: 'absolute', top: '70%', right: '10%', width: '400px', borderRadius: '16px', y: img2Y, rotate: img2Rotate, filter: 'blur(0px) brightness(0.8)', boxShadow: '0 30px 60px rgba(0,0,0,0.6)' }} />
          
          {/* Middle Left, super fast */}
          <motion.img src={alampat} style={{ position: 'absolute', top: '40%', left: '15%', width: '250px', borderRadius: '16px', y: img3Y, rotate: img3Rotate, filter: 'blur(1px) brightness(0.6)', boxShadow: '0 15px 30px rgba(0,0,0,0.4)' }} />
          
          {/* Very Bottom Right, huge and slow */}
          <motion.img src={hcdc} style={{ position: 'absolute', top: '90%', right: '25%', width: '500px', borderRadius: '16px', y: img4Y, rotate: img4Rotate, filter: 'blur(4px) brightness(0.5)', boxShadow: '0 25px 50px rgba(0,0,0,0.7)' }} />

          {/* Top Right, fast */}
          <motion.img src={skonnect} style={{ position: 'absolute', top: '20%', right: '5%', width: '280px', borderRadius: '16px', y: img5Y, rotate: img5Rotate, filter: 'blur(1px) brightness(0.7)', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }} />
          
          {/* Extra: Center Left, medium */}
          <motion.img src={compass} style={{ position: 'absolute', top: '60%', left: '5%', width: '320px', borderRadius: '16px', y: img5Y, rotate: img1Rotate, filter: 'blur(3px) brightness(0.4)', boxShadow: '0 20px 40px rgba(0,0,0,0.6)' }} />
        </motion.div>
        ...
        <div style={{
          ...
          {/* Timeline Node 1 */}
          <motion.div style={{ opacity: opacity1, scale: scale1, y: y1, position: 'absolute', textAlign: 'center' }}>
            <h2 style={{ fontSize: '4.5rem', fontWeight: 900, color: 'var(--text-primary)', textShadow: '0 10px 30px rgba(0,0,0,0.8)', margin: 0, textTransform: 'uppercase', letterSpacing: '4px' }}>
              My Perspective
            </h2>
            <p className="text-gradient" style={{ fontSize: '1.5rem', fontWeight: 600, marginTop: '1rem', letterSpacing: '2px' }}>
              Turning Complexity into Clarity
            </p>
          </motion.div>

          {/* Timeline Node 2 */}
          <motion.div style={{ opacity: opacity2, scale: scale2, y: y2, position: 'absolute', textAlign: 'center' }}>
            <h2 style={{ fontSize: '4.5rem', fontWeight: 900, color: 'var(--text-primary)', textShadow: '0 10px 30px rgba(0,0,0,0.8)', margin: 0, textTransform: 'uppercase', letterSpacing: '4px' }}>
          ...
          {/* Timeline Node 3 */}
          <motion.div style={{ opacity: opacity3, scale: scale3, y: y3, position: 'absolute', textAlign: 'center' }}>
            <h2 style={{ fontSize: '5rem', fontWeight: 900, color: 'var(--accent-primary)', textShadow: '0 0 20px var(--accent-glow)', margin: 0, textTransform: 'uppercase', letterSpacing: '8px' }}>
              Explore Everything
            </h2>
            <p style={{ fontSize: '1.2rem', color: 'var(--text-secondary)', marginTop: '2rem', letterSpacing: '4px', textTransform: 'uppercase' }}>
              Scroll down to view my projects ↓
            </p>
            {/* Scroll Progress Indicator */}
            <div style={{ width: '500px', maxWidth: '80vw', height: '4px', background: 'rgba(255, 255, 255, 0.1)', margin: '1.5rem auto 0', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--surface-border)' }}>
